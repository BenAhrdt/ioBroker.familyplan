...
exports.stableSegment = stableSegment;
exports.stableSegments = stableSegments;
exports.eventFolder = eventFolder;
exports.eventObjectId = eventObjectId;
 ...
 *
 */
function stableSegment(label, existing) {
    ...
    return `${base}_${(0, node_crypto_1.createHash)("sha1").update(label).digest("hex").slice(0, 8)}`;
}
/** Resolve all segments independent of input order, including real collisions. */
function stableSegments(labels) {
    ...
    for (const [base, labelsInGroup] of groups) {
        ...
        for (const [index, label] of group.entries()) {
            result.set(label, index === 0
                ? base
                : `${base}_${(0, node_crypto_1.createHash)("sha1").update(label).digest("hex").slice(0, 8)}`);
        }
    }
    return result;
}
 ...
 *
 */
function eventFolder(event, segments) {
    const rawEventType = event.event_type;
    const eventType = segments?.eventTypes?.get(rawEventType) ?? normalizeId(rawEventType);
    if (rawEventType.toUpperCase() !== "OTHER") {
        return `events.appointment.${eventType}`;
    }
    const rawCustomType = typeof event.custom_type_label === "string"
 ...
 *
 */
function eventObjectId(event, idSegment = normalizeId(String(event.id))) {
    if (event.id != null) {
        return `event_${idSegment}`;
    }
    return `generated_${(0, node_crypto_1.createHash)("sha1")
        .update([
        event.child_id ?? "",
